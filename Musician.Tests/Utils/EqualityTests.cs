...
namespace Musician.Tests.Utils {
    public static class EqualityTests {
        ...
        private static TestResult TestEqualityOperator<T>(T obj1, T obj2, bool expectedEqual, MethodInfo equalityOperator) {
            return SafeCall("Operator ==", () => {
                bool equal = (bool)equalityOperator.Invoke(null, new object[] { obj1, obj2 });
        ...
        private static TestResult TestInequalityOperator<T>(T obj1, T obj2, bool expectedUnequal, MethodInfo inequalityOperator) {
            return SafeCall("Operator !=", () => {
                bool unequal = (bool)inequalityOperator.Invoke(null, new object[] { obj1, obj2 });
        ...
        private static TestResult SafeCall(string functionName, Func<TestResult> test) {
            try {
                ...
                string message = string.Format("{0} threw {1}: {2}", functionName, ex.GetType().Name, ex.Message);
                return TestResult.CreateFailure(message);
            }
        }
        ...
        private static MethodInfo GetOperator<T>(string methodName) {
            Type[] types = new Type[2] { typeof(T), typeof(T) };
            MethodInfo equalityOperator = typeof(T).GetMethod(methodName, types);
            if (!equalityOperator.IsStatic && !equalityOperator.IsPublic)
                throw new Exception("Operator must be public and static");
        ...
        private static void AssertAllTestsHavePassed(IList<TestResult> testResults) {
            ...
            string[] errors = testResults.Where(r => !r.IsSuccess).Select(r => r.ErrorMessage).ToArray();
